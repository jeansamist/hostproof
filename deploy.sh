...
# Re-export so ecosystem.config.cjs picks them up
...
log "  Node ${NODE_VER} / pnpm $(pnpm --version) / pm2 $(pm2 --version)"

# ─── 2. Env file check ───────────────────────────────────────────────────────
log "Checking API env file…"
if [[ ! -f "apps/api/.env" ]]; then
  if [[ -f "apps/api/.env.example" ]]; then
    warn "apps/api/.env not found — copying from .env.example"
    cp apps/api/.env.example apps/api/.env
    warn "Please edit apps/api/.env with your real values and re-run this script."
    exit 1
  else
    die "apps/api/.env not found. Create it from .env.example before deploying."
  fi
fi
success "API env file present"
...
if [[ "$SKIP_BUILD" == false ]]; then
  ...
  warn "--skip-build set: reusing existing build artifacts"
fi

# ─── 5. Prepare API build directory ─────────────────────────────────────────
log "Preparing API build directory…"

if [[ ! -d "apps/api/build" ]]; then
  die "apps/api/build/ not found. Run without --skip-build first."
fi

# Copy .env into the build dir so AdonisJS can find it at runtime
cp apps/api/.env apps/api/build/.env
success ".env copied to apps/api/build/"

# Install production-only deps inside the build folder
...
if [[ "$SKIP_MIGRATE" == false ]]; then
  ...
  warn "--skip-migrate set: skipping database migrations"
fi
...
if pm2 list | grep -q "hostproof-api\|hostproof-web"; then
  log "Existing PM2 processes found — reloading (zero downtime)…"
  pm2 reload ecosystem.config.cjs --env production
else
  log "No existing PM2 processes — starting fresh…"
...
echo -e "  ${BOLD}pm2 startup${RESET}       — auto-start on system reboot"
echo ""
